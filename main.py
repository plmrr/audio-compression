...
from algorithms import dct_from_scratch, idct_from_scratch, dft, idft, lagrange_approximation
import numpy as np
...
def main():
    ...
    if method == 'DCT':
        ...
        degree = int(len(signal) * (1 - threshold_percent))
        print(degree)
        reconstructed_signal = lagrange_approximation(signal, degree)
    else:
    ...
    # Plot the signals
    if method in ('DCT', 'DFT'):
        plot_signals(signal, transformed_signal, compressed_signal, reconstructed_signal, sample_rate, method, mse_value, threshold_percent)
    else:
        plot_signals(original=signal, reconstructed=reconstructed_signal, sample_rate=sample_rate, method=method, mse=mse_value, compression=threshold_percent)
